...
fn captures_text_tool_call_and_usage() {
    ...
    // Native Claude Code tool calls are executed inside the CLI's own loop and
    // must not be surfaced to OpenHuman's harness, which owns its tools via
    // MCP. The mapper still consumes the block and its input deltas so neither
    // half leaks into the harness stream.
